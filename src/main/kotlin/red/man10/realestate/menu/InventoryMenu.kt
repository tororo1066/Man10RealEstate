...
object InventoryMenu {
    ...
    fun mainMenu(p: Player) {
        ...
        if (lan(p)){
            val inventory = CustomInventory.createInventory(9, "§a§lMain Menu")

            inventory.setItem(1, CustomInventory.IS(Material.PAPER, "§f§lTake care of the land you can manage"))
            inventory.setItem(4, CustomInventory.IS(Material.NETHER_STAR, "§f§l"))
            inventory.setItem(7, CustomInventory.IS(Material.STONE, "Check out the land you've liked"))
    ...
    fun openBookmark(p: Player, page: Int) {
        if (lan(p)){
            val inventory = CustomInventory.createInventory(54, "§a§lLiked list")
            ...
            if (inventory.getItem(44) != null) {
                ...
                val next = CustomInventory.IS(Material.LIGHT_BLUE_STAINED_GLASS_PANE, "§6§lNext page")
            ...
            if (page != 0) {
                val next = CustomInventory.IS(Material.LIGHT_BLUE_STAINED_GLASS_PANE, "§6§lPrevious page")
    ...
    fun openRegionList(p: Player, page: Int) {
        if (lan(p)){
            val inventory = CustomInventory.createInventory(54, "§a§lLand listings")

            val list = User.ownerList[p]

            if (list.isNullOrEmpty()) {
                sendMessage(p, "§c§lYou have no land of your own")
    ...
    fun regionMenu(p: Player, id: Int) {
        ...
        if (lan(p)){
            val inventory = CustomInventory.createInventory(27, "§a§lSetting out the land")

            inventory.setItem(0, back)
            inventory.setItem(11, CustomInventory.IS(Material.PAPER, "§f§lDetailed land settings", mutableListOf(
                    "§f§lCurrent settings",
            ...
            inventory.setItem(13, CustomInventory.IS(Material.PLAYER_HEAD, "§b§lResident management", mutableListOf(), id))
            inventory.setItem(15, CustomInventory.IS(Material.EMERALD_BLOCK, "§a§lAdd a resident", mutableListOf(), id))
    ...
    fun regionSetting(p: Player, id: Int) {
        ...
        if (lan(p)){
            val inventory = CustomInventory.createInventory(54, "§6§lDetailed land settings")
            ...
            inventory.setItem(10, CustomInventory.IS(Material.COMPASS, "§e§lStatus", mutableListOf("§aCurrent status：${rg.status}"), id))
            inventory.setItem(13, CustomInventory.IS(Material.EMERALD, "§e§lPricing",
                    mutableListOf("§eCurrent rates：${String.format("%,.1f", rg.price)}"), id))

            inventory.setItem(16, CustomInventory.IS(Material.ENDER_PEARL, "§a§lTeleportation settings", mutableListOf("§aMake the current position the teleport point"), id))

            inventory.setItem(38, CustomInventory.IS(Material.CLOCK, "§b§lRental span setting",
                    mutableListOf("§aThe currently set span：${
                        when (rg.span) {
                            0 -> "Per month"
                            1 -> "Per week"
                            else -> "Per day"
            ...
            inventory.setItem(42, CustomInventory.IS(Material.PLAYER_HEAD, "§3§lChange of owner", mutableListOf(), id))
    ...
    fun statusMenu(p: Player, id: Int) {
        ...
        if (lan(p)){
            val inventory = CustomInventory.createInventory(9, "§a§lChange of status")
            ...
            inventory.setItem(1, CustomInventory.IS(Material.RED_STAINED_GLASS_PANE, "§c§lThe Lawless Zone", mutableListOf("§f§lRemove the protection"), id))
            inventory.setItem(3, CustomInventory.IS(Material.LIME_WOOL, "§a§lFree", mutableListOf("§f§lCan do more than just place and destroy blocks"), id))
            inventory.setItem(5, CustomInventory.IS(Material.EMERALD, "§e§lOn sale", mutableListOf(), id))
            inventory.setItem(7, CustomInventory.IS(Material.IRON_DOOR, "§c§lProtection", mutableListOf(), id))
    ...
    fun spanMenu(p: Player, id: Int) {
        ...
        if (lan(p)){
            val inventory = CustomInventory.createInventory(9, "§a§lChange of span")
            ...
            inventory.setItem(1, CustomInventory.IS(Material.ENDER_PEARL, "§c§lPer day", mutableListOf(), id))
            inventory.setItem(4, CustomInventory.IS(Material.ENDER_PEARL, "§b§lPer week", mutableListOf(), id))
            inventory.setItem(7, CustomInventory.IS(Material.ENDER_PEARL, "§a§lPer month", mutableListOf(), id))
    ...
    fun userList(p: Player, id: Int, page: Int) {
        ...
        if(lan(p)){
            val inventory = CustomInventory.createInventory(54, "§a§lList of inhabitants")
            ...
            GlobalScope.launch {
                ...
                if (list == null) {
                    sendMessage(p, "§c§lThe land seems to be uninhabited.")
                    CustomInventory.close(p)
                    return@launch
                }
                ...
                for (d in list) {
                    ...
                    meta.lore = mutableListOf(
                            ...
                            "§7§lStatus:${userData.status}",
                            "§8§lRent:${userData.rent}"
                    )
    ...
    fun userMenu(p: Player, id: Int, uuid: UUID) {
        ...
        if (lan(p)){
            val inventory = CustomInventory.createInventory(27, "§6§lSetting of ${Bukkit.getOfflinePlayer(uuid).name}")
            ...
            inventory.setItem(11, CustomInventory.IS(Material.RED_STAINED_GLASS_PANE, "§3§lPermission settings", mutableListOf(), uuid, id))
            inventory.setItem(13, CustomInventory.IS(Material.EMERALD, "§a§lSet your rent", mutableListOf(), uuid, id))
            inventory.setItem(15, CustomInventory.IS(Material.REDSTONE_BLOCK, "§4§lEvicting residents", mutableListOf(), uuid, id))
    ...
    fun setPermission(p: Player, id: Int, uuid: UUID) {
        ...
        if (lan(p)){
            val inventory = CustomInventory.createInventory(54, "§a§lAdvanced permission settings")
            ...
            GlobalScope.launch {
                ...
                }, "§3§lAll rights reserved", mutableListOf(), uuid, id))

                inventory.setItem(22, CustomInventory.IS(if (data.allowBlock) {
                    Material.LIME_STAINED_GLASS_PANE
                } else {
                    Material.RED_STAINED_GLASS_PANE
                }, "§3§lPlacement and destruction of blocks", mutableListOf(), uuid, id))

                inventory.setItem(31, CustomInventory.IS(if (data.allowInv) {
                    Material.LIME_STAINED_GLASS_PANE
                } else {
                    Material.RED_STAINED_GLASS_PANE
                }, "§3§lOpen chests and other inventory", mutableListOf(), uuid, id))

                inventory.setItem(40, CustomInventory.IS(if (data.allowDoor) {
                    Material.LIME_STAINED_GLASS_PANE
                } else {
                    Material.RED_STAINED_GLASS_PANE
                }, "§3§lRight and left clicks on doors etc. (except signs)", mutableListOf(), uuid, id))
